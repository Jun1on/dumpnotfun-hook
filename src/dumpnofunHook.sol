...
contract dumpnofunHook is BaseHook{
    ...
    bool private initialLiquidityDeposited;
    ...
    event AwesomePoolTokenCreated(PoolId id, address tokenAddress, string name, string symbol, uint8 decimals, uint256 totalSupply);


    // Initialize BaseHook parent contract in the constructor
    constructor(IPoolManager _poolManager) BaseHook(_poolManager) {
		manager = _poolManager;
        initialLiquidityDeposited = false;
    }
    ...
    function deployPool(TokenParams calldata params) external returns (PoolKey memory, SimpleERC20Token token) {

        SimpleERC20Token token = new SimpleERC20Token(msg.sender, params);
        Currency ETH = Currency.wrap(address(0));
        PoolKey memory key = PoolKey(ETH, Currency.wrap(address(token)), 3000, 60, this);
        ...
        BalanceDelta delta = abi.decode(
            manager.unlock(abi.encode(CallbackData(msg.sender, key, liquidityParams))),
            (BalanceDelta)
        );

        PoolId poolId = key.toId();
        (uint128 liquidityAfter1,,) = manager.getPositionInfo(poolId, address(this), -1020, 0, 0);
        (uint128 liquidityAfter2,,) = manager.getPositionInfo(poolId, msg.sender, -1020, 0, 0);
        
        emit AwesomePoolTokenCreated(poolId, address(token), params.name, params.symbol, params.decimals, params.totalSupply);
    ...
    function withdrawLiquidity(PoolKey memory key, IPoolManager.ModifyLiquidityParams memory liquidityParams) external returns (PoolKey memory) {
        BalanceDelta delta = abi.decode(
            manager.unlock(abi.encode(CallbackData(msg.sender, key, liquidityParams))),
            (BalanceDelta)
        );
    ...
    function _unlockCallback(bytes calldata rawData) internal virtual override returns (bytes memory){
        ...
        if (initialLiquidityDeposited) {
            require(data.sender == lockupAddress[data.key.toId()] && block.number >= lockupEndDateForPool[data.key.toId()], "Liquidity is locked");
        } else {
            initialLiquidityDeposited = true;
        }
        ...
        if (delta0 < 0) data.key.currency0.settle(manager, data.sender, uint256(-delta0));
        if (delta1 < 0) data.key.currency1.settle(manager, data.sender, uint256(-delta1));

        if (delta0 > 0) data.key.currency0.take(manager, data.sender, uint256(delta0));
        if (delta1 > 0) data.key.currency1.take(manager, data.sender, uint256(delta1));
    ...
    function settle(Currency currency, IPoolManager manager, address payer, uint256 amount) internal {
        ...
        if (currency.isAddressZero()) {
            ...
            if (payer != address(this)) {
                SimpleERC20Token(Currency.unwrap(currency)).transferFrom(payer, address(manager), amount);
            } else {
                SimpleERC20Token(Currency.unwrap(currency)).transfer(address(manager), amount);
            }
            manager.settle();
        }
    }
